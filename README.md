...
| **設定を開く**         | `config.toml` を既定のアプリケーションで開く                                     |
...
監視モード常駐時に、アクティブなウィンドウを問わず以下のホットキーが使用できます（`config.toml` の `hotkeys` で変更可能。反映には再起動が必要です）。
...
- **有効化**: トレイメニューの「履歴」→「履歴機能を有効にする」で切り替えます（`config.toml` の `history_enabled` でも設定可能）。
...
設定ファイルの解析に失敗した場合、元ファイルは `config.toml.bak` として退避され、デフォルト設定で起動します。

TOML 形式のため `#` でコメントを書ける。初回保存時は各項目の説明コメントが付与され、以降の保存ではユーザーが追記したコメントを維持したまま値のみ更新される。

### 設定項目

| キー                                  | 型     | デフォルト      | 説明                                                 |
| :------------------------------------ | :----- | :-------------- | :--------------------------------------------------- |
| `version`                             | number | `1`             | 設定スキーマのバージョン                             |
