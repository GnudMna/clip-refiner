...
Invoke-ScriptMain {
    ...
    # -p: ワークスペース内の clip-refiner クレートを対象にする
    # -I: wix/main.wxs はルート直下 (クレート manifest とは別パス)
    # --nocapture: cargo build の rustc エラーを表示する (省略時は exit 101 だけになる)
    $wixArgs = @('-p', 'clip-refiner', '-I', 'wix/main.wxs', '--nocapture')
    if ($SkipBuild) {
        $wixArgs += '--no-build'
    }
    cargo wix @wixArgs
...
上記の rustc / linker のメッセージを確認するか、次を手動で実行してください:
  ...
  rustc --version
  cargo build --release -p clip-refiner
